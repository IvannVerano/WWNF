...
namespace Zenon {
    ...
    Ataque::Ataque(GameDataRef l_data, sf::Vector2f l_posicion, sf::Texture &l_textura, const std::vector<Enemy*> &l_enemigos, int l_precio, int l_porcentaje, int l_rango, int l_potencia, float l_cadencia, int l_refresco, int l_id, float l_appearing) : Trampa(l_data), m_enemy(l_enemigos) {
    ...
    bool Ataque::CheckColision() {
        ...
        {
            for (int i = 0; i < m_bala.size(); i++) {
                if (m_bala[i] != NULL && m_target != -1 && m_bala[i]->GetSprite().getGlobalBounds().intersects(m_enemy[m_target]->GetSprite().getGlobalBounds())) {
                    delete m_bala[i];
                    m_bala.erase(m_bala.begin() + i);
                    m_enemy.at(m_target)->TakeDamage(m_potencia);
                    if (m_enemy[m_target]->GetActualState() == ENEMY_STATE_DEAD) {
                        m_enemy[m_target]->SetKiller(m_id);
    ...
    void Ataque::DeleteTarget(int l_target) 
    {
        if (m_target == l_target)
            m_target = -1;
    }
}